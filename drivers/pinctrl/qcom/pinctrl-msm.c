...
#if IS_ENABLED (CONFIG_HPM_DEBUG)
#include <linux/his_debug_base.h>

static inline bool check_gpio_is_protected(unsigned gpio)
{
	int i = 0;

	for (i = 0; i < dev_bi.prot_num; i++) {
		if (gpio == dev_bi.prot_gpios[i])
			return true;
	}

	return false;
}
#endif /* CONFIG_HPM_DEBUG */
...
struct gpio_config_info {
	...
	unsigned int drive;
	unsigned int pull;
	unsigned int intr_cfg;
	unsigned int direct;
	unsigned int value;
};

static inline void gpio_get_config_info(struct msm_pinctrl *pctrl, int idx,
		struct gpio_config_info *gpio_cfg)
{
	...
	g = &pctrl->soc->groups[idx];
	config = msm_readl_ctl(pctrl, g);
	io_val = msm_readl_io(pctrl, g);
	intr_val = msm_readl_intr_cfg(pctrl, g);

	gpio_cfg->intr_cfg = intr_val & BIT(g->intr_enable_bit);
	gpio_cfg->pull = (config >> g->pull_bit) & 3;
	gpio_cfg->drive = (config >> g->drv_bit) & 7;
	gpio_cfg->func = (config >> g->mux_bit) & 7;
	gpio_cfg->direct = !!(config & BIT(g->oe_bit));

	if (gpio_cfg->direct)
		gpio_cfg->value = !!(io_val & BIT(g->out_bit));
	else
		gpio_cfg->value = !!(io_val & BIT(g->in_bit));
}

int print_all_gpio_status(struct seq_file *m, void *v)
{
	int i;
	struct gpio_config_info gpio_cfg = {0};
	struct msm_pinctrl *pctrl = msm_pinctrl_data;

	if (!pctrl) {
		pr_err("Error, the msmgpio_pctrl is NULL\n");
		return 0;
	}

	PRINT_OUT(m, "\n");
	PRINT_OUT(m, " id \t\t func \t\t direct \t value \t pull \t\t drv_strenth \t int_cfg\n");
	for (i = 0; i < pctrl->soc->ngpios; i++) {
		if (check_gpio_is_protected(i))
			continue;

		gpio_get_config_info(pctrl, i, &gpio_cfg);
		PRINT_OUT(m, " GPIO_%d   \t %d \t\t %s \t\t %d\t %s\t %s\t %s\n",
					i,
					gpio_cfg.func,
					gpio_dir[gpio_cfg.direct],
					gpio_cfg.value,
					gpio_pull[gpio_cfg.pull],
					drv_strength_str[gpio_cfg.drive],
					gpio_int_en[gpio_cfg.intr_cfg]);
	}
	PRINT_OUT(m, "\n");
	return 0;
}
