...
#ifdef CONFIG_CHARGE_FG_FUNCTION
		dev_err(&led->client->dev,
				"dev_bi.board_id=%d, led->cdev.name %s\n",
				dev_bi.board_id, led->cdev.name);
		if (dev_bi.board_id > 1) {
			if (strcmp(led->cdev.name, "green") == 0)
				led->cdev.name = "blue";
				...
				const struct i2c_device_id *id)
{
	...
	led_array->no_regulator= of_property_read_bool(node,
				"aw2023,no-regulator");

	dev_err(&client->dev, "%s:no_regulator is %d\n",
					__func__, led_array->no_regulator);
