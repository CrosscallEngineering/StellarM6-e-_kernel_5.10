...
int wlfw_cap_send_sync_msg(struct icnss_priv *priv)
{
	...
	case WCN6750:
		strlcpy(wcn_chipid, "WCN6750", sizeof("WCN6750"));
		break;
	case WCN3990:
	...
	case WCN3950:
		...
		break;
	default:
		strlcpy(wcn_chipid, "WCNXXXX", sizeof("WCNXXXX"));
		break;
	}
	...
	icnss_pr_warn("Capability, chip_id: 0x%x, chip_family: 0x%x, board_id: 0x%x, soc_id: 0x%x",
		     priv->chip_info.chip_id, priv->chip_info.chip_family,
		     priv->board_id, priv->soc_id);

	icnss_pr_warn("fw_version: 0x%x, fw_build_timestamp: %s, fw_build_id: %s",
		     priv->fw_version_info.fw_version,
