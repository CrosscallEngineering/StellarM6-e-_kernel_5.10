...
#if IS_ENABLED(CONFIG_ROAM_SETTING)||IS_ENABLED(CONFIG_WMMPS_SETTING)
static const char *cmdline;
static int icnss_get_cmdline(void) {
	...
	if (ret) {
		icnss_pr_err("%s: Can't not parse bootargs, ret=%d\n", __func__, ret);
	}
	return ret;
}
#endif/* CONFIG_ROAM_SETTING || CONFIG_WMMPS_SETTING */

/*added for roam Threshold settings begin by H-WADRV*/
#if IS_ENABLED(CONFIG_ROAM_SETTING)
int roam_threshold_value = -1;
int roam_diff_value = -1;
int roam_diff6ghz_value = -1;

static int parse_cmdline_roam(void) {
    const char * tar_str1 = "roam_threshold_boot=";
    const char * tar_str2 = "roam_diff_boot=";
    const char * tar_str3 = "roam_diff6ghz_boot=";
    int ret = 0;
    //parse roam_threshold_boot
    char * pos = strstr(cmdline, tar_str1);
    if (pos) {
        if (sscanf(pos, "roam_threshold_boot=%d", &roam_threshold_value) <= 0) {
            icnss_pr_err("%s: sscanf is wrong!\n", __func__);
            ret = 1;
        }
    } else {
        icnss_pr_err("%s: not find tar_str1!\n", __func__);
        ret = 1;
    }
    icnss_pr_info("%s: roam_threshold_value = %d\n", __func__, roam_threshold_value);
    //parse roam_diff_boot
    pos = strstr(cmdline, tar_str2);
    if (pos) {
        if (sscanf(pos, "roam_diff_boot=%d", &roam_diff_value) <= 0) {
            icnss_pr_err("%s: sscanf is wrong!\n", __func__);
            ret = 1;
        }
    } else {
        icnss_pr_err("%s: not find tar_str2!\n", __func__);
        ret = 1;
    }
    icnss_pr_info("%s: roam_diff_value = %d\n", __func__, roam_diff_value);
    //parse roam_diff6ghz_boot
    pos = strstr(cmdline, tar_str3);
    if (pos) {
        if (sscanf(pos, "roam_diff6ghz_boot=%d", &roam_diff6ghz_value) <= 0) {
            icnss_pr_err("%s: sscanf is wrong!\n", __func__);
            ret = 1;
        }
    } else {
        icnss_pr_err("%s: not find tar_str3!\n", __func__);
        ret = 1;
    }
    icnss_pr_info("%s: roam_diff6ghz_value = %d\n", __func__, roam_diff6ghz_value);
    return ret;
}

int roam_threshold_get(void)
{
       return roam_threshold_value;
}
EXPORT_SYMBOL(roam_threshold_get);

static ssize_t roam_threshold_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	if (sscanf(buf, "%d", &roam_threshold_value) != 1) {
		icnss_pr_err("%s: sscanf is wrong!\n", __func__);
		return -EINVAL;
	}
	icnss_pr_info("%s: %d\n", __func__, roam_threshold_value);
...
static ssize_t roam_threshold_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", roam_threshold_value);
}
static struct kobj_attribute roam_threshold_attr = __ATTR_RW(roam_threshold);

int roam_diff_get(void)
{
       return roam_diff_value;
}
EXPORT_SYMBOL(roam_diff_get);

static ssize_t roam_diff_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	if (sscanf(buf, "%d", &roam_diff_value) != 1) {
		icnss_pr_err("%s: sscanf is wrong!\n", __func__);
		return -EINVAL;
	}
	icnss_pr_info("%s: %d\n", __func__, roam_diff_value);
...
static ssize_t roam_diff_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", roam_diff_value);
}
static struct kobj_attribute roam_diff_attr = __ATTR_RW(roam_diff);

int roam_diff6ghz_get(void)
{
       return roam_diff6ghz_value;
}
EXPORT_SYMBOL(roam_diff6ghz_get);

static ssize_t roam_diff6ghz_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	if (sscanf(buf, "%d", &roam_diff6ghz_value) != 1) {
		icnss_pr_err("%s: sscanf is wrong!\n", __func__);
		return -EINVAL;
	}
	icnss_pr_info("%s: %d\n", __func__, roam_diff6ghz_value);
...
static ssize_t roam_diff6ghz_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", roam_diff6ghz_value);
}
static struct kobj_attribute roam_diff6ghz_attr = __ATTR_RW(roam_diff6ghz);

static struct attribute *roam_setting_attrs[] = {
	&roam_threshold_attr.attr,
	&roam_diff_attr.attr,
	&roam_diff6ghz_attr.attr,
	NULL,
};

static struct attribute_group roam_setting_attr_group = {
	.attrs = roam_setting_attrs,
};
#endif /* CONFIG_ROAM_SETTING */
/*added for roam Threshold settings end by H-WADRV*/

/*added for wmm-ps settings begin by H-WADRV*/
#if IS_ENABLED(CONFIG_WMMPS_SETTING)
int wmmps_uapsd_value = -1;

static int parse_cmdline_wmmps(void) {
    const char * tar_str = "wmmps_uapsd_boot=";
    int ret = 0;
    //parse wmmps_uapsd_boot
    char * pos = strstr(cmdline, tar_str);
    if (pos) {
        if (sscanf(pos, "wmmps_uapsd_boot=%d", &wmmps_uapsd_value) <= 0) {
            icnss_pr_err("%s: sscanf is wrong!\n", __func__);
            ret = 1;
        }
    } else {
        icnss_pr_err("%s: not find tar_str!\n", __func__);
        ret = 1;
    }
    icnss_pr_info("%s: wmmps_uapsd_value = %d\n", __func__, wmmps_uapsd_value);

    return ret;
}

int wmmps_uapsd_get(void)
{
       return wmmps_uapsd_value;
}
EXPORT_SYMBOL(wmmps_uapsd_get);

static ssize_t wmmps_uapsd_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	if (sscanf(buf, "%d", &wmmps_uapsd_value) != 1) {
		icnss_pr_err("%s: sscanf is wrong!\n", __func__);
		return -EINVAL;
	}
	icnss_pr_info("%s: %d\n", __func__, wmmps_uapsd_value);
...
static ssize_t wmmps_uapsd_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", wmmps_uapsd_value);
}
static struct kobj_attribute wmmps_uapsd_attr = __ATTR_RW(wmmps_uapsd);

static struct attribute *wmmps_setting_attrs[] = {
	&wmmps_uapsd_attr.attr,
	NULL,
};

static struct attribute_group wmmps_setting_attr_group = {
	.attrs = wmmps_setting_attrs,
};
#endif /* CONFIG_WMMPS_SETTING */
/*added for wmm-ps settings end by H-WADRV*/
...
static int icnss_probe(struct platform_device *pdev)
{
	...
	/*added for roam Threshold settings begin by H-WADRV*/
#if IS_ENABLED(CONFIG_ROAM_SETTING)
	ret = his_register_sysfs_attr_group(&roam_setting_attr_group);
	if (ret < 0) {
		icnss_pr_err("%s: Error creating roam sysfs node\n", __func__);
	}
	icnss_pr_info("%s: create roam sysfs node successfully\n", __func__);
#endif /* CONFIG_ROAM_SETTING */
	/*added for roam Threshold settings end by H-WADRV*/

	/*added for wmm-ps settings begin by H-WADRV*/
#if IS_ENABLED(CONFIG_WMMPS_SETTING)
	ret = his_register_sysfs_attr_group(&wmmps_setting_attr_group);
	if (ret < 0) {
		icnss_pr_err("%s: Error creating wmmps sysfs node\n", __func__);
	}
	icnss_pr_info("%s: create wmmps sysfs node successfully\n", __func__);
#endif /* CONFIG_WMMPS_SETTING */
	/*added for wmm-ps settings end by H-WADRV*/
