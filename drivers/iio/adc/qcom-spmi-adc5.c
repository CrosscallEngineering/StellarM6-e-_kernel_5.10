...
struct iio_dev *pmi_iio_dev = NULL;
struct iio_dev *pmic_iio_dev = NULL;

EXPORT_SYMBOL(pmi_iio_dev);
EXPORT_SYMBOL(pmic_iio_dev);
...
#if IS_ENABLED (CONFIG_HQPNP_REG_DEBUG)
static const char *regard_adc_name[] = {
    "bat_therm"
};
#endif /*CONFIG_HQPNP_REG_DEBUG*/
			 ...
			 long mask, adc_do_conversion do_conv)
{
	...
	case IIO_CHAN_INFO_PROCESSED:
		...
		for(i = 0; i < ARRAY_SIZE(regard_adc_name); i++) {
			if(!strcmp(prop->datasheet_name, regard_adc_name[i]))
				break;
			if(i == ARRAY_SIZE(regard_adc_name)-1)
				pr_err("%s, dname:%s, temp|vol:%d, [%s UTC]\n", __func__, prop->datasheet_name, *val, time_str);
		}
#endif /*CONFIG_HQPNP_REG_DEBUG*/
...
#if IS_ENABLED (CONFIG_HQPNP_REG_DEBUG)
	if(strstr(indio_dev->name, "pm7250b") != 0)
		pmi_iio_dev = indio_dev;
	else if(strstr(indio_dev->name, "pmk8350") != 0)
		pmic_iio_dev = indio_dev;
#endif /*CONFIG_HQPNP_REG_DEBUG*/
