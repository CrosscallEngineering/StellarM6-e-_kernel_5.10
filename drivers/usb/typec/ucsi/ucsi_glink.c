			    ...
			    struct ucsi_connector_status *status, size_t len)
{
	...
	if (cmd_requested && offset == UCSI_MESSAGE_IN) {
		...
		dev_err(udev->dev, "%s : conn_partner_type %d\n",__func__, conn_partner_type);
...
#if IS_ENABLED(CONFIG_DOUBLE_CHARGERS)
		entry->constat_info.partner_otg = true;
#endif/*CONFIG_DOUBLE_CHARGERS*/
...
#if IS_ENABLED(CONFIG_DOUBLE_CHARGERS)
			if(!conn_partner_type && entry->constat_info.partner_otg)
				entry->constat_info.partner_otg = false;
#endif/*CONFIG_DOUBLE_CHARGERS*/
			break;
		}
...
#if IS_ENABLED(CONFIG_DOUBLE_CHARGERS)
static struct ucsi_dev *g_udev = NULL;
void ucsi_usb_role_switch(void){
	if(!g_udev)
		return;
	cancel_delayed_work_sync(&g_udev->usb_role_work);
	schedule_delayed_work(&g_udev->usb_role_work, msecs_to_jiffies(1000));
	return;
}
EXPORT_SYMBOL_GPL(ucsi_usb_role_switch);
static void ucsi_usb_role_work(struct work_struct *work){
	struct ucsi_dev *udev  = container_of(work, struct ucsi_dev,
			usb_role_work.work);
	struct constat_info_entry *entry;
	int ret = 0;
	...
	INIT_LIST_HEAD(&entry->node);
	if(entry->constat_info.acc != TYPEC_ACCESSORY_NONE ||
		!entry->constat_info.partner_usb)
		return;
	if(entry->constat_info.partner_otg)
		u_role = USB_ROLE_HOST;
	else
		u_role = USB_ROLE_DEVICE;
	ret = usb_role_switch_set_role(udev->ucsi->connector->usb_role_sw, u_role);
	if (ret)
		dev_err(udev->dev, "%s(): failed to set role(%d):%d\n",
						__func__, u_role, ret);
	return;
}
#endif /*CONFIG_DOUBLE_CHARGERS*/
...
#if IS_ENABLED(CONFIG_DOUBLE_CHARGERS)
	INIT_DELAYED_WORK(&udev->usb_role_work, ucsi_usb_role_work);
#endif/*CONFIG_DOUBLE_CHARGERS*/
...
static int ucsi_remove(struct platform_device *pdev)
{
	...
	ucsi_destroy(udev->ucsi);
#if IS_ENABLED(CONFIG_DOUBLE_CHARGERS)
	cancel_delayed_work_sync(&udev->usb_role_work);
#endif/*CONFIG_DOUBLE_CHARGERS*/
