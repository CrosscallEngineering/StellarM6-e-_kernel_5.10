			...
			if (st21nfc_dev->buffer[0] == IDLE_CHARACTER) {
				if (st21nfc_dev->buffer[1] == IDLE_CHARACTER) {
					if (st21nfc_dev->buffer[2] == IDLE_CHARACTER) {
						pr_err("%s Read 7E7E7E... header, IRQ always high ? Stop\n",
							__func__);
						break;
					} else {
						...
						len = i2c_master_recv(st21nfc_dev->client,
									...
									2);
						if (len != 2) {
							pr_err("%s Could not read rest of header after 7E7E: %d\n",
								__func__, len);
							break;
						}
						len = i2c_master_recv(st21nfc_dev->client,
						...
						if (len != (int)st21nfc_dev->buffer[2]) {
							pr_err("%s Could not read payload: %d\n",
								__func__, len);
							break;
						}
					}
					...
					len = i2c_master_recv(st21nfc_dev->client,
								...
								1);
					if (len != 1) {
						pr_err("%s Could not read payload length: %d\n",
							__func__, len);
						break;
					}
					len = i2c_master_recv(st21nfc_dev->client,
...
static void st21nfc_ping_work(struct work_struct *data)
{
#ifdef WITH_PING_DURING_PROBE
	struct st21nfc_device *st21nfc_dev =
		container_of(data, struct st21nfc_device, ping_work.work);
	int ret = 0;
...
// QCOM and MTK54 use standard GPIO definition
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
		...
		if(!st21nfc_dev->st_p_wq) {
			pr_err("%s, creat workqueue failed\n", __func__);
			return -ENODEV;
		}
	...
	INIT_DELAYED_WORK(&st21nfc_dev->ping_work, st21nfc_ping_work);
	...
	schedule_delayed_work(&st21nfc_dev->ping_work, msecs_to_jiffies(500));
