...
#ifdef CONFIG_PRODUCT_DEVINFO
    productinfo_register(PRODUCTINFO_FINGERPRINT_ID, "focaltech TF9391EC-N12", NULL);
#endif /* CONFIG_PRODUCT_DEVINFO */
    FF_LOGD("'%s' leave.", __func__);
    return 0;
...
int ff_suspend_resume(bool is_suspend)
{
    char *uevent_env[2];
    bool this_screen_on;
    ff_context_t *ff_ctx;
    ...
    FF_LOGD("'%s' enter.", __func__);
    if (g_ff_ctx == NULL) {
        FF_LOGD("'%s' g_ff_ctx is null, return.", __func__);
        return 0;
    }
    ff_ctx = g_ff_ctx;
    if (is_suspend) {
...
EXPORT_SYMBOL(ff_suspend_resume);
