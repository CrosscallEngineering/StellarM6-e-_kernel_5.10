...
bool check_firmware_fs_mounted(void)
{
    struct path root_path;
    struct path path;
    int ret=-1;
    static bool firmware_fs_mounted = false;

    if(!firmware_fs_mounted) {
		...
		#if defined(CONFIG_TOUCHSCREEN_CHIPONE_IN_QGKI)
	    ret = kern_path(CFG_CTS_FIRMWARE_FILEPATH, LOOKUP_FOLLOW, &path);
		#endif
	    if (ret) {
			path_put(&root_path);
			return false;
		...
		firmware_fs_mounted = !!ret;

	    path_put(&path);
	    path_put(&root_path);
    }

    return firmware_fs_mounted;
}
