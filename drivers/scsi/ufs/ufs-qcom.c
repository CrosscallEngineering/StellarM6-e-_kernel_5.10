...
#ifdef CONFIG_PRODUCT_DEVINFO
char ufs_product_info[130];
char ufs_more[90];

static int get_ufs_size(struct ufs_hba *hba)
{
	int ret;
	u8 total_size[QUERY_DESC_MAX_SIZE];
	u64 total_size_s;
	u64 ufs_size = 0;
	...
	total_size_s = ((u64)total_size[GEOMETRY_DESC_PARAM_DEV_CAP + 7] << 0 |
			(u64)total_size[GEOMETRY_DESC_PARAM_DEV_CAP + 6] << 8 |
			(u64)total_size[GEOMETRY_DESC_PARAM_DEV_CAP + 5] << 16 |
			(u64)total_size[GEOMETRY_DESC_PARAM_DEV_CAP + 4] << 24 |
			(u64)total_size[GEOMETRY_DESC_PARAM_DEV_CAP + 3] << 32 |
			(u64)total_size[GEOMETRY_DESC_PARAM_DEV_CAP + 2] << 40 |
			(u64)total_size[GEOMETRY_DESC_PARAM_DEV_CAP + 1] << 48 |
			(u64)total_size[GEOMETRY_DESC_PARAM_DEV_CAP + 0] << 56);
	if (ret) {
	...
	printk("Device ufs size is %ld\n", total_size_s);
	ufs_size = (total_size_s * 512) / (1024*1024*1024);
	if (ufs_size <= 8)
		return 8;
	else if (ufs_size <= 16)
		return 16;
...
static int get_ddr_size(void)
{
	...
	__kernel_ulong_t total = totalram_pages();
		 ...
		 */
	ddr_size = 	((total) << (PAGE_SHIFT - 10))/(1024*1024);
	if(((total) << (PAGE_SHIFT - 10))%(1024*1024) > 0)
		ddr_size++;
	printk("%s:total = %lu,ddr_size = %d.\n",__func__,total,ddr_size);
	return ddr_size;
}
 
int ufshcd_read_manufacture_date(struct ufs_hba *hba)
{
	int err = 0;
...
static int ufs_read_health_desc_data(struct ufs_hba *hba)
{
	...
	pm_runtime_get_sync(hba->dev);
	err = ufshcd_read_desc_param(hba, QUERY_DESC_IDN_HEALTH, 0, 0, desc_buf, 
	...
	if (!err) {
		...
		health_info.pre_elo_info = (u8)desc_buf[tmp->offset];
		
		tmp = &health_desc_field_name[3];
		health_info.dev_life_time_A= (u8)desc_buf[tmp->offset];

		tmp = &health_desc_field_name[4];
		health_info.dev_life_time_B= (u8)desc_buf[tmp->offset];
	}
	return 0;
}
...
static inline char ufshcd_qcom_remove_non_printable(u8 ch)
{
 ...
 */
int ufshcd_qcom_read_string_desc(struct ufs_hba *hba, u8 desc_index,
			    u8 **buf, bool ascii)
{
	...
	if (ascii) {
		...
		// replace non-printable or non-ASCII characters with spaces 
		for (i = 0; i < ret; i++)
			str[i] = ufshcd_qcom_remove_non_printable(str[i]);
...
static int ufs_read_psn(struct ufs_hba *hba, uint8_t* psn, uint8_t buf_size) {
	int ret = 0;
	uint8_t index = 0, valid_string_len = 0;
	uint8_t *desc_buf, *psn_buf;
	...
	//get Serial Number String in ascii
	ret = ufshcd_qcom_read_string_desc(hba, index, &desc_buf, SD_ASCII_STD);
	if(ret < 0)
		goto out;

	//check if need to re-get Serial Number String in raw
	if(strstr(desc_buf, " ")){
		pr_err("[%s,%d] Serial Number contains unprintable ascii, retry to read in raw\n", __func__, __LINE__);
		if((ret=ufshcd_qcom_read_string_desc(hba, index, &desc_buf, SD_RAW)) < 0)
			goto out;
		//convert raw hex to ascii for human reading
		valid_string_len = ret - QUERY_DESC_HDR_SIZE;
		if(NULL == (psn_buf=kzalloc(valid_string_len*2+1, GFP_ATOMIC)))
			return -ENOMEM;
		bin2hex(psn_buf, desc_buf+QUERY_DESC_HDR_SIZE, valid_string_len);
		ret = snprintf(psn, min((uint8_t)(valid_string_len*2+1),buf_size), "%s", psn_buf);
		kfree(psn_buf);
	}
	else
		ret = snprintf(psn, ret, "%s", desc_buf);
out:
	kfree(desc_buf);
	return ret;
}
...
void ufs_add_ufsinfo_to_productinfo(struct ufs_hba *hba)
{
	...
	mdt_month = (manufacture_date >> 8 & 0xff);
	mdt_year = (manufacture_date  & 0xff);

	if((ret=ufs_read_psn(hba, psn, sizeof(psn))) < 0) {
		pr_err("[%s, %d] ufs_read_psn fail, err=%d\n", __func__, __LINE__, ret);
		memset(psn, 0, sizeof(psn));
		snprintf(psn, sizeof("ERROR"), "%s", "ERROR");
	}

	//snprintf(psn, sizeof("ERROR"), "%s", "ERROR");
	snprintf(ufs_product_info, sizeof(ufs_product_info),
			"STOR_SIZE:%dG(DDR)+%dG(UFS),MNM %.8s,PNM %.16s,PSN %s,FWV %.4s,MDT 20%.2x/%.2x PQ: %d",
			ddr_size,ufs_size,sdev->vendor,sdev->model,psn,sdev->rev,mdt_year,mdt_month,sdev->inq_periph_qual);
	life_time_a = health_info.dev_life_time_A;
	life_time_b = health_info.dev_life_time_B;
	if (life_time_a < 0xb) {
	...
	switch(health_info.pre_elo_info) {
		case 0:
			eol_str = "Not Defined";
			break;
		case 1:
...
static int ufs_info_proc_show(struct seq_file *m, void *v)

{
	...
	ret=ufshcd_read_manufacture_date(hba);
	if (ret){
		printk("ERROR:read manufacture date,ret= %d\n",ret);
	}
	ufs_add_ufsinfo_to_productinfo(hba);
    seq_printf(m, "UFS: %s\nUFS more: %s\n", ufs_product_info,ufs_more);
	return ret;
}
static int ufs_info_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, ufs_info_proc_show, PDE_DATA(inode));
}

static const struct proc_ops ufs_info_proc_fops = {
	.proc_open           = ufs_info_proc_open,
	.proc_read           = seq_read,
...
static int ufs_qcom_init_sysfs(struct ufs_hba *hba)
{
	...
	if (ret)
		...
		//struct proc_dir_entry *prEntry;
		 proc_create_data("ufs_info", 0444, NULL, &ufs_info_proc_fops, hba->dev);
		//	if (!prEntry) {
		//		pr_err("%s,failed to create node: /proc/ufs_info\n",__func__);
		//	}
