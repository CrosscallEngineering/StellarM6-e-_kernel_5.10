...
/* Jira L78002-356: added for otg-tfupdate, avoid adsp backup ko loaded fail when vendor damaged.
 * Force ignore crc fail for adsp kos in tfupdate mode.
*/
#if IS_ENABLED(CONFIG_IGNORE_MODCRC_MISMATCH)
static bool tfupdate_mode = false;

static int __init tfupdate_mode_check(char *s)
{
	if (!strcmp(s, "tfupdate"))
		tfupdate_mode = true;
	else
		tfupdate_mode = false;

	pr_err("%s: buffer= %s, tfupdate_mode= %d\n", __func__, s, tfupdate_mode);
	return 0;
}
__setup("androidboot.mode=", tfupdate_mode_check);
#endif /*CONFIG_IGNORE_MODCRC_MISMATCH*/
...
bad_version:
#if IS_ENABLED(CONFIG_IGNORE_MODCRC_MISMATCH) 
	if(tfupdate_mode && (!strcmp(info->name, "q6_pdr_dlkm") ||!strcmp(info->name, "q6_notifier_dlkm") \
				|| !strcmp(info->name, "snd_event_dlkm") || !strcmp(info->name, "gpr_dlkm") || !strcmp(info->name, "adsp_loader_dlkm") \
				|| !strcmp(info->name, "audio_prm_dlkm") || !strcmp(info->name, "q6_dlkm") || !strcmp(info->name, "spf_core_dlkm"))){
		pr_warn("tfupdate: force IGNORE %s disagrees about version of symbol %s\n",
				info->name, symname);
		return 1;
	}
	else
